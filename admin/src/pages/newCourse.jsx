...
export default () => {
    ...
    const postData = async (datapost) => {
        ...
        const { data, status } = await repository.updateCourse(datapost,datapost._id).then(x => x).then(x => x)
    ...
    return <div>
        ...
        <Navbar title="Module" />
        <div className={`${classes.root} ${classes.margin}`}>{"Nouveau module"}</div>
        <div >
        <div style={{padding: '0 27%'}} >
        <Formik
                                initialValues={{
                                    _id:location.state&&location.state.data&&location.state.data._id?location.state.data._id:"",
                                    name: location.state&&location.state.data&&location.state.data.name?location.state.data.name:"",
                                    image: location.state&&location.state.data.image&&location.state.data.image?location.state.data.image:"",
